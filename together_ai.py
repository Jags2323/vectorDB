...
def main(file_path, prompt):
    ...
    # Check if relevant paragraphs contain AI-related information
    if not relevant_paragraphs:
        print("No relevant paragraphs found.")
        return

    # Use Together API to get a response from a larger model
    api_key = os.getenv("TOGETHER_API_KEY")  # Ensure the Together API key is set in the environment variable
    ...
    if response.choices:
        complete_answer = response.choices[0].text.strip()
        finish_reason = response.choices[0].finish_reason
        
        while finish_reason == "length":
            full_prompt += complete_answer
            response = client.completions.create(
                model="meta-llama/Llama-3-70b-chat-hf",  # Use the larger model from Together API
                prompt=full_prompt,
                max_tokens=512,
                temperature=0.7,
                top_p=0.7,
...
if __name__ == "__main__":
    ...
    parser.add_argument('prompt', type=str, help='Query prompt')
